use crate::{midi::ChannelMessageType, model::Model};
use std::iter;
        ...
        match &self.model {
            Ok((model, temperament)) => {
                html! {
                    ...
                    >
                        <div>{ "Click on the buttons or use the keyboard to play notes. Use up/down to change the waveform. If you experience audio buffer underruns (crackling noise) increase the buffer size." }</div>
                        <div>
